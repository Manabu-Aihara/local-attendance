...
from app.models import User, Shinsei, RecordPaidHoliday, CountAttendance, TimeAttendance
...
class AttendanceAnalysys:
    ...
    def analysys(self):
        shinseis = Shinsei.query.filter_by(STAFFID=self.staffid).all()
        STAFFID = self.staffid
        shin = Shinsei.query.filter_by(STAFFID=self.staffid)
        u = User.query.get(self.staffid)
        rp_holiday = RecordPaidHoliday.query.get(self.staffid)
        cnt_attemdance = CountAttendance.query.get(self.staffid)
        tm_attendance = TimeAttendance.query.get(self.staffid)
        ...
        if self.c.strftime("%Y-%m-%d") == self.data1 and sh:  # 登録上書き
            ...
            if (
                self.data2 == "00:00" and self.data3 != "00:00"
            ):  #################################
            ...
            ##### 届出別条件分岐4
                ...
                (int(self.data2.split(":")[0]) == int(self.data3.split(":")[0]))
                and (int(self.data2.split(":")[1]) > int(self.data3.split(":")[1]))
                or (int(self.data2.split(":")[0]) > int(self.data3.split(":")[0]))
                and (
                    int(self.data2.split(":")[0]) != 0
                    and int(self.data3.split(":")[0] != 0)
                )
            ):
            ...
            elif (self.data7 == "4" and self.data11 != "4") or (
                self.data7 == "16" and self.data11 != "16"
            ):
                if self.data11 == "6":
                    flash("保存しました。", "success")
                    start_time = "00:00"
            ...
            if (
                ...
                and not (
                    self.data7 == ""
                    and self.data11 == ""
                    and self.data2 == "00:00"
                    and self.data3 == "00:00"
                )
            ) and jpholiday.is_holiday_name(self.c):
                sh.HOLIDAY = holiday
                db.session.commit()
            elif (
                (
            ...
            ) and get_day_of_week_jp(self.c) == "1":
                sh.HOLIDAY = holiday
                db.session.commit()
            else:
                sh.HOLIDAY = holiday
            ...
            if self.data7 == "7":
                ...
                dm = monthmod(
                    inday.replace(month=4, day=1), d_input.replace(month=4, day=1)
                )[0].months

                if inday.month >= 4 and inday.month <= 7:
                    if d_input >= inday and d_input < inday.replace(
                        month=4, day=1
                        ...
                        for shs in shinseis:
                            ...
                            if d >= inday and d < inday.replace(
                                month=4, day=1
                            ) + relativedelta(months=12):
                    ...
                    elif d_input >= inday.replace(month=4, day=1) + relativedelta(
                        months=dm
                    ) and d_input < inday.replace(month=4, day=1) + relativedelta(
                        months=dm + 12
                    ):
                        # reflesh = 3
                        ...
                        for shs in shinseis:
                            dd = shs.WORKDAY
                            d = datetime.strptime(dd, "%Y-%m-%d")
                            if d >= inday.replace(month=4, day=1) + relativedelta(
                                months=dm
                            ) and d < inday.replace(month=4, day=1) + relativedelta(
                                months=dm + 12
                            ):
                                if shs.NOTIFICATION == "7":
                                    n = n + 1

...
from app.forms import SaveForm, SelectMonthForm
...
def indextime(STAFFID):
    ...
    if "y" in session:
        ...
        if datetime.today().month > 2:
            if datetime.today().year > int(session["y"]) or (
                datetime.today().year == int(session["y"])
                and datetime.today().month > int(session["m"]) + 1
            ):
                # dsp_page = "pointer-events: none;"
                dsp_page = ""
        elif datetime.today().month == 1:
            if datetime.today().year > int(session["y"]) or (
                datetime.today().year - 1 == int(session["y"])
                and int(session["m"]) <= 11
            ):
                # dsp_page = "pointer-events: none;"
                dsp_page = ""
        else:
    ...
    if shinseis is None:
        ...
        if User.query.get(STAFFID) is None:
            ...
            if inday is not None:
                ...
                # 新入職員
                if (
                    datetime.today() < giveday
                    and monthmod(datetime.today(), giveday)[0].months < 6
                ):
                    last_giveday = inday  # 今回付与
                    next_giveday = giveday  # 次回付与
                elif (
                    datetime.today() > giveday
                    and monthmod(giveday, datetime.today())[0].months < 6
                ):
                ...
                ##### 新規登録職員 #####
                if (
                    rp_holiday.LAST_DATEGRANT is None
                    and rp_holiday.NEXT_DATEGRANT is None
                ):  # 新規登録職員＝今回付与、次回付与のない状態
                            ...
                            if rp_holiday.LAST_DATEGRANT == "":
                                if (
                                    ...
                                    if shs.NOTIFICATION == "3":
                                        ...
                                        shs.NOTIFICATION == "4"
                                        or shs.NOTIFICATION == "16"
                                    ):
                                        nenkyu_half_days += 0.5

                                    if (
                                        shs.NOTIFICATION2 == "4"
                                        or shs.NOTIFICATION2 == "16"
                                    ):
                                        nenkyu_half_days += 0.5
                            else:
                                if (
                        ...
                        for shs in shinseis:
                            if (
                                ...
                                if shs.NOTIFICATION == "3":
                                    ...
                                    shs.NOTIFICATION == "4" or shs.NOTIFICATION == "16"
                                ):
    ...
    shins = Shinsei.query.filter(
        Shinsei.STAFFID == STAFFID, Shinsei.WORKDAY >= FromDay, Shinsei.WORKDAY <= ToDay
    ).all()
